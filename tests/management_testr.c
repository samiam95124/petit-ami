/*******************************************************************************
*                                                                             *
*              WINDOW MANAGEMENT TEST PROGRAM, ROOTED DISPLAY                 *
*                                                                             *
*                    Copyright (C) 2005 Scott A. Moore                        *
*                                                                             *
* Tests text and graphical windows management calls on a rooted display,      *
* where the program window is the root surface (the frame buffer backend).    *
* The root cannot move, size or change Z order -- there is nothing behind     *
* it -- so it serves as the desktop: every test is applied to a child         *
* window, the way management_testc applies them on the character mode         *
* window manager. Tests that create child windows make them children of      *
* the test window, a child of a child.                                        *
*                                                                             *
*******************************************************************************/
...

{
    ...
    if (!keeptitle) {
        ...
        sprintf(titlebuf, "management_testr: frame %d", framenum);
...

{
    ...
    /* A reference window on the desktop to flip against: the desktop is
       the root window, so the test provides the neighbor. */
    ami_openwin(&stdin, &win2, NULL, 3);
    ...
    do {
        ...
        } else if (er.etype == ami_etterm && er.winid == 2)
