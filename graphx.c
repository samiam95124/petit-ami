...
typedef struct pa_menurec{
        ...
        boolean    onoff; /* on/off highlight */
        boolean    oneof; /* "one of" highlight */
        boolean    bar; /* place bar under */
        int        id; /* id of menu item */
        string     face;  /* text to place on button */
...
static Display *padisplay;
static Window pawindow;
static int pascreen;
...

{
    ...
    while (1) {
        ...
        if (e.type == Expose) {
            ...
            XDrawString(padisplay, pawindow, DefaultGC(padisplay, pascreen), 10,
                        50, msg, strlen(msg));
...

{
    ...
    fprintf(stderr, "The character is: %c\n", c);
