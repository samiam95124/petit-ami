...
buffer resident in memory, which the CPU can then develop pixel level routines
...
kept to that function and an overrider routine exists to place a new vector to
that function as well as keep the previous contents of the vector. That last is
important because it allows the overrider to implement what is commonly referred
...
The basis for this is that each layer can be built solely using the functional
API exposed by the module below it.

## Acceleration

Many graphics cards have hardware acceleration features, in terms of figure
drawing, block pixel moves, 3d triangles, frame buffer flipping and others.

These accelerators are not necessarily better. Sometimes the CPU can beat them
