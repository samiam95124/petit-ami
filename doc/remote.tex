...
\texttt{event()} returns the next inbound event. It needs no threads.
...
The client needs no threads at all. Calls go out on the command channel as
they are made, in order. \texttt{event()} reads the event channel. A query
sends its request and waits for the reply before anything else is issued.
...
The server's event pump thread calls \texttt{event()} on the display
library without pause, translating each event to the wire record and
sending it. The client's \texttt{event()} reads the event channel,
translates back, and returns the event to the program, replacing the wire
handle with the window identification the API promises.
